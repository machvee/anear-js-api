...
class AnearApi extends ApiService {
  ...
  async uploadAppCss(appId, cssContent, fileSuffix) {
    logger.debug(`API: POST upload app styles ${appId} with suffix ${fileSuffix}`)
    ...
    const attrs = json.data.attributes
    logger.debug('uploadAppCss response:', attrs)
    return attrs
  }
  ...
  async uploadAppImageAssets(appId, b64EncodedGzippedTarBall) {
    ...
    const attrs = json.data.attributes
    logger.debug('uploadAppImageAssets response:', attrs)
    return attrs
  }
