...
class AnearMessaging {
  ...
  async closeParticipant(eventId, participantId, callback) {
    ...
    await this.getAnearEventWithLockFromStorage(
      ...
      async (anearEvent) => {
        ...
        this.detachParticipantPrivateChannel(eventId, anearParticipant.userId)
  ...
  detachParticipantPrivateChannel(eventId, userId) {
  ...
  async participantActionTimedOutMessagingCallback(eventId, message) {
    ...
    logger.debug(`participant TIMED OUT!(${eventId}, ${participantId})`)
  ...
  publishEventCssMessage(eventId, userId, message, callback) {
    ...
    this.publishEventPrivateMessage(eventId, userId, message, 0, callback, CssDisplayMessageType)
  }

  publishEventPrivateMessage(eventId, userId, message, timeoutMilliseconds=0, callback=null, messageType=PrivateDisplayMessageType) {
    const channel = this._eventChannels[eventId].privates[userId]
    ...
    this.publishChannelMessage(channel, messageType, payload, callback)
  }
  ...
  checkMessageHistory (channel, messageType, callback) {
    channel.history({untilAttach: true}, (err, resultPage) => {
      if (err) {
        ...
        if (resultPage.items.length > 0) logger.info(resultPage.items.length + ` ${messageType} history events received in page`)
