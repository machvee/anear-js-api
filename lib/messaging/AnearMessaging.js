...
class AnearMessaging {
  ...
  async refreshActiveParticipants(anearEvent) {
    return Promise.all(
      // rehydrate the anearEvent participants and host
      anearEvent.participants.ids.map(
        async participant => {
          const participant = await this.AnearParticipantClass.getFromStorage(participantId)
          // need to load the participants
          this.processParticipantEnter(
            anearEvent,
            participant.id,
