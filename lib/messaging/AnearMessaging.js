...
class AnearMessaging {
  ...
  async initEventRealTimeMessaging(anearEvent) {
    ...
    const channels = this.eventChannels[anearEvent.id]

    channels.spectators = anearEvent.allowsSpectators() ? await this.setupSpectatorsChannel(anearEvent) : null
    channels.events = await this.setupEventsChannel(anearEvent)
    channels.participants = await this.setupParticipantsChannel(anearEvent)
    channels.actions = await this.setupActionsChannel(anearEvent)
