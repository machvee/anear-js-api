...
class AnearMessaging {
  ...
  getParticipants(eventId) {
    return new Promise(
      (resolve, reject) => {
        this.eventChannels[eventId].actions.presence.get(
          (err, messages) => {
            if (err) {
              reject(err)
            } else {
              resolve(messages) // messages: [{data: {<participant identity}}]
            }
          }
        )
      }
  ...
  async participantEnterMessagingCallback(eventId, message) {
    ...
    logger.debug(`**** ENTER PARTICIPANT ****  event: ${eventId}, participant: ${message.data.id}`)
    ...
    try {
      ...
      logger.debug(`API fetch participant info for ${message.data.name}`)
  ...
  detachParticipantPrivateChannel(eventId, anearParticipant) {
    ...
    if (channel) {
      this.detachChannel(this.eventChannels[eventId].privates[userId])
  ...
  publishEventParticipantsMessage(eventId, participants, message, timeoutMilliseconds=0, callback=null) {
    const channel = this.eventChannels[eventId].participants
    const payload = {
      content: message,
      timeout: timeoutMilliseconds
    }
    let timerCallback = callback

    if (timeoutMilliseconds > 0) {
      timerCallback = () => {
        this.setMultipleParticipantTimers(eventId, participants, timeoutMilliseconds)
  ...
  detachAll(eventId) {
    ...
    this.detachChannel(channels.events)
    this.detachChannel(channels.participants)
    this.detachChannel(channels.actions)
    this.detachChannel(channels.spectators)
    Object.values(channels.privates).forEach(channel => {
