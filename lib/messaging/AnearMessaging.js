...
class AnearMessaging {
  ...
  async participantEnterMessagingCallback(eventId, message) {
    ...
    try {
      ...
      const eventLockCallback = async (anearEvent) => {
        ...
        await anearEvent.participantEnter(anearParticipant, message.data)
        await anearEvent.update()
      }
  ...
  async publishEventSpectatorsMessage(eventId, message) {
    const channel = this.eventChannels[eventId].spectators
    const payload = {content: message}

    await this.publishChannelMessage(channel, PublicDisplayMessageType, payload)
  }

  async publishEventCssMessage(eventId, anearParticipant, message) {
  ...
  async detachAll(eventId) {
    ...
    await this.detachChannel(channels.spectators)
    Object.values(channels.privates).forEach(
      async channel => await this.detachChannel(channel)
    )
  }
