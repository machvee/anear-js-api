...
class AnearEvent extends JsonApiResource {
  ...
  get xStateConfig() {
    //
    // override in subclass if xState HSM is the desired
    // form of async messaging from participants and application
    // transitioning
    return null
  }

  get xStateOptions() {
    //  override in subclass with an Xstate actions, guards, etc lookup objects
    return {}
  }
  ...
  async participantEnter(anearParticipant) {
    ...
    if (this.participants.exists(anearParticipant)) {
      ...
      if (this.hasXstate()) {
        this.xStateSend(RefreshEvent, {anearParticipant})
      } else {
        await this.participantRefreshEventCallback(anearParticipant)
      }
  ...
  async participantAction(anearParticipant, actionEventName, actionPayload) {
    ...
    if (hasXstate()) {
      this.xStateSend(actionEventName, {anearParticipant, ...actionPayload})
    } else {
  ...
  xStateInit() {
    if (this.hasXstate()) {
      this.xStateMachine = createMachine(this.xStateConfig, this.xStateOptions)
      this.currentXstate = State.create(this.previousXstate ? this.previousXstate : this.xStateMachine.initialState)
    }
  }
  ...
  xStateSend(eventName, params) {
    ...
    this.currentXstate = this.xStateMachine.transition(this.currentXstate, sendEvent)
    this.currentXstate.actions.forEach((action) => {
      typeof action.exec === 'function' && action.exec(this.currentXstate.context, sendEvent)
    })
  }
}
