...
class AnearEvent extends JsonApiResource {
  ...
  async participantEnterEventCallback(anearParticipant) {
    throw new Error('You have to implement participantEnterEventCallback() in your AnearEvent sub-class');
  }

  async participantRefreshEventCallback(anearParticipant) {
    throw new Error('You have to implement participantRefreshEventCallback() in your AnearEvent sub-class');
  }

  async spectatorRefreshEventCallback() {
    throw new Error('You have to implement spectatorRefreshEventCallback() in your AnearEvent sub-class');
  }

  async participantCloseEventCallback(userId) {
    throw new Error('You have to implement participantCloseEventCallback() in your AnearEvent sub-class');
  }

  async participantActionEventCallback(message) {
    throw new Error('You have to implement participantActionEventCallback() in your AnearEvent sub-class');
  }

  async participantTimedOutEventCallback(message) {
    throw new Error('You have to implement participantTimedOutEventCallback() in your AnearEvent sub-class');
  }

  async eventBroadcastEventCallback(message) {
    throw new Error('You have to implement eventBroadcastEventCallback() in your AnearEvent sub-class');
  }



  isParticipantUserEventCreator(anearParticipant) {
  ...
  publishEventParticipantsMessage(message, callback=null, timeoutMilliseconds=0) {
  ...
  publishEventPrivateMessage(anearParticipantUserId, message, callback=null, timeoutMilliseconds=0) {
    return this.messaging.publishEventPrivateMessage(
      this.id,
      anearParticipantUserId,
      message,
      callback,
      timeoutMilliseconds
    )
  ...
  async participantClose(anearParticipant) {
    ...
    if (this.hasParticipant(anearParticipant)) {
      ...
      anearParticipant.remove()
    }
  }

  async participantAction(userId, actionJSON) {
    await this.participantActionEventCallback(userId, actionJSON)
  }

  async participantTimedOut(userId) {
    await this.participantTimedOutEventCallback(userId)
  }
