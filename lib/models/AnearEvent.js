...
class AnearEvent extends JsonApiResource {
  ...
  updateEventContext(assignFunc) {
    this.anearStateMachine.updateContext(assignFunc)
  }
  ...
  async getClonedEvent() {
    ...
    if (!conedEventData) return null

    const clonedEvent = await this.getFromStorage(clonedEventData.id)
    return clonedEvent
  }
