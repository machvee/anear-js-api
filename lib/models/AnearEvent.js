...
class AnearEvent extends JsonApiResource {
  ...
  addParticipant(anearParticipant) {
    const identity = anearParticipant.identity

    identity.isHost = this.attributes.hosted && this.isParticipantEventCreator(anearParticipant)
  ...
  activeParticipants() {
    return Object.values(this.participants).filter(p => !p.isHost)
  }
