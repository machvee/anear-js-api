...
class AnearEvent extends JsonApiResource {
  ...
  async publishSpectatorCss() {
    await this.messaging.publishSpectatorsCssMessage(
      this.id,
      this.css
    )
  }
  ...
  async participantEnter(anearParticipant) {
    ...
    if (this.participants.hasParticipant(anearParticipant)) {
      logger.info(`participant ${anearParticipant.id} exists.  Refreshing...`)

      this.participants.addParticipant(this, anearParticipant) // update the participants entry
      await this.participantRefreshEventCallback(anearParticipant)
      await anearParticipant.update()
    } else {
      this.participants.addParticipant(this, anearParticipant) // add the participants entry
  ...
  async participantClose(anearParticipant) {
    ...
    //
    this.participants.purgeParticipant(anearParticipant)
