...
class AnearEvent extends JsonApiResource {
  ...
  async onLoad() {
    await super.onLoad()
    //this.participants = await this.messaging.getParticipants(this.id)
  }
  ...
  hasParticipant(anearParticipant) {
    return typeof this.getEventParticipantById(anearParticipant.id) !== "undefined"
  }

  getEventParticipantById(anearParticipantId) {
    return this.participants.find(p => p.data.id == anearParticipantId)
  }
  ...
  activeParticipants() {
    return this.participants.filter(p => !p.data.isHost)
  }
  ...
  publishCss(anearParticipant, callback) {
    this.messaging.publishEventCssMessage(
      this.id,
      anearParticipant,
      this.css,
      callback
