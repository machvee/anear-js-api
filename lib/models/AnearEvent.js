...
class AnearEvent extends JsonApiResource {
  ...
  async render(xContext, xEvent, renderings) {
    // run all renderings (display requests) in a Promise.all
    //   participant: render to a single pariticpant's private channel
    //   participants: render to a all (other) pariticpant's public channel (except private request participants)
    //   spectators: render to the spectator channel
    //
    //     participant: AnearParticipant record of user triggering event
    //     template: path to pug template file
    //     timeout: response timeout in seconds
    //
    // e.g.
    //  await this.render(context, event, {
    //    participant: {template: "app/views/battleChessParticipant.pug", timeout: secs},
    //    participants: {template: "app/view/batleChessPlayers.pug", timeout: secs},
    //    spectators: {template: "app/views/spectatorBoard.pug"},
    //  })
    //
    //  The pug template function will have the following context passed:
    //    participant: current participant metadata
    //    context: stateMachineContext
    //    event: stateMachine event payload e.g. ("MOVE": {x: 7, y: 3})
    //    anearEvent metadata
    //    anearEvent participants
    //
    this.view.render(context, event, renderings)
  }
