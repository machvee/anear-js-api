...
class AnearEvent extends JsonApiResource {
  ...
  constructor(json, messaging)  {
    ...
    this.xStateInit(json.previousXstate)
  }
  ...
  toJSON() {
    return {
      ...
      previousXstate: this.currentXstate.toJSON()
    }
  }
  ...
  xStateInit(previousXstate) {
    if (this.hasXstate()) {
      this.xStateMachine = createMachine(this.xStateConfig, this.xStateOptions)
      const startingState = previousXstate ? previousXstate : this.xStateMachine.initialState
      this.currentXstate = State.create(startingState)
    }
  }
  ...
  xStateSend(eventName, params) {
    ...
    const newState = this.xStateMachine.transition(this.currentXstate, sendEvent)

    this.currentXstate = newState
    this.currentXstate.actions.forEach((action) => {
      typeof action.exec === 'function' && action.exec(this.xStateMachine.context, sendEvent)
    })
  }
}
