...
class AnearEvent extends JsonApiResource {
  ...
  async saveEvent(appmContext) {
    // Save current AppM context to ANAPI developer endpoint
    try {
      await anearApi.saveAppEventContext(this.id, appmContext)
      logger.info(`Saved App Event Context for event ${this.id}`)
    } catch (e) {
      logger.error('Failed to save app event context', e)
      throw e
    }
  }
