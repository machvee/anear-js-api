"use strict"
const { createMachine, State, interpret } = require('xstate')
...
class AnearEvent extends JsonApiResource {
  ...
  constructor(json, messaging)  {
    ...
    this.xStateInit() // optionally initializes/restores xState machine
  }
  ...
  toJSON() {
    return {
      ...
      previousXstate: JSON.stringify(this.currentXstate)
    }
  }
  ...
  async participantEnter(anearParticipant) {
    ...
    if (this.participants.exists(anearParticipant)) {
      ...
      // if xState, instead of the participantRefreshEventCallback,
      // this should send an JOIN xState event to the gameMachine with anearParticipant
      // as an event attribute
      // https://xstate.js.org/docs/guides/states.html#persisting-state for persising
      // state.  Don't require the app dev to manage the state.   We can store it 
      // with the Event outside of the appData
      if (this.hasXstate()) {
        this.xStateSend(JoinEvent, {anearParticipant})
      } else {
        await this.participantRefreshEventCallback(anearParticipant)
      }
  ...
  async participantClose(anearParticipant) {
    ...
    // if xState, this should send an CLOSE xState event to the gameMachine with anearParticipant
    // as an event attribute
    //
    this.participants.purge(anearParticipant)
    if (this.hasXstate()) {
      this.xStateSend(CloseEvent, {anearParticipant})
    } else {
  ...
  async participantAction(anearParticipant, actionEventName, actionPayload) {
    // if xState, instead of invoking the participantActionEventCallback,
    // this should send an anearEventName xState event to the gameMachine with anearParticipant
    // and actionPayload as event attributes
    if (hasXstate()) {
      this.xStateSend(actionEventName, {anearParticipant, payload: actionPayload})
    } else {
  ...
  async participantTimedOut(anearParticipant) {
    // if xState, instead of the invoking the participantTimedOutEventCallback, this should send
    // a TIMEOUT xState event to the gameMachine with anearParticipant as an event attribute
    if (hasXstate()) {
      this.xStateSend(TimeoutEvent, {anearParticipant})
    } else {
  ...
  xStateInit() {
    if (this.hasXstate()) {
      this.xStateMachine = createMachine(this.xStateConfig).withContext(this.appData)
      this.currentXstate = json.previousXstate ? State.create(json.previousXstate) : this.xStateMachine.initialState
      this.xStateService = interpret(this.xStateMachine).
                             onTransition(state => logger.debug(`transitioning to ${state.value}`)).
                             start(this.currentXstate)
    }
  }

  hasXstate() {
    //
    // App devs who inherit from AnearEvent can provide an
    // xState Configuration in this.xStateConfig. This will
    // trigger the sending of xState Events rather than the default
    // invocation of messaging callbacks.
    return this.xStateConfig
  }

  xStateSend(eventName, params) {
    this.currentXstate = this.xStateService.send(
      {
        type: eventName,
        ...params
      }
