...
class JsonApiResource {
  ...
  async onCreate() {
    return // called once when event created.  override in subclass if an async initialization is required
  }
  ...
  static async getWithLockFromStorage(modelId, callback, ...args) {
    return await this.getJsonWithLockFor(modelId, async json => {
      if (!json) return null
  ...
  static async getJsonWithLockFor(id, callback) {
    const key = this.persistKey(id)
    return Storage.lockedFetch(key, callback)
  }
}
