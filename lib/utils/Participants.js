...
class Participants {
  ...
  reloadFromStorage(callback) {
    ...
    return Promise.all(
      ids.map(
        p => {
          const participant = this.addFromStorage(participantId)
  ...
  async addFromStorage(participantId) {
    // load from storage and return AnearParticipant instance to caller
    const participant = await this.anearEvent.anearParticipantClass.getFromStorage(participantId, this.anearEvent)
  ...
  load(participantsJsonArray) {
    // useful in test scripts to populate a Participants
  }
}
