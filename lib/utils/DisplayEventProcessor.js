  ...
  _processSingle(displayEvent) {
    ...
    switch (appRenderContext.meta.viewer) {
      ...
      case 'participant':
        ...
        return this._processPrivateParticipantDisplays(template, templateRenderContext, timeoutFn)
  ...
  _processPrivateParticipantDisplays(template, templateRenderContext, timeoutFn) {
    Object.values(this.participantsIndex.all).forEach(
      participantStruct => {
        ...
        if (timeoutFn) renderMessage.timeout = timeoutFn(templateRenderContext.app, participantId)
