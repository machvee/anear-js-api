 ...
 *  - Adds exit flag to display events when event is in closing state.
...
const AppMachineTransition = (anearEvent) => {
  return (appEventMachineState) => {
    ...
    if (metaObjects.length > 0) {
      ...
      // Check if event is in closing state and add exit flag
      const isEventClosing = anearEvent.state?.matches('activeEvent.closeEvent')
