...
const MetaViewPathEventProcessing = (anearEvent) => {
  return (appEventMachineState) => {
    ...
    const xStateName = Stringified(appEventMachineState.value)
    const fullyQualifiedStateName = `${appEventMachineState.machine.id}.${xStateName}`
    const meta = rawMeta[fullyQualifiedStateName]

    if (!meta) return

    logger.debug(`Meta ${fullyQualifiedStateName}: `, meta)
    logger.debug("Context: ", appEventContext)
    logger.debug("AnearEventState: ", anearEvent.anearEventMachine.state.value)
    ...
    const processParticipantDisplay = (meta, anearEventMachine) => {
      ...
      logger.debug("processParticipantDisplay: ", meta)
      ...
      Object.values(anearParticipantMachines).forEach((machine) => {
        ...
        logger.debug("participantState.state/context: ", participantState.value, participantState.context)

        if (participantState.value.matches(C.ActiveStateName)) {
