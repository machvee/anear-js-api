    ...
    active: {
      ...
      states: {
        ...
        live: {
          ...
          states: {
            idle: {
              entry: [(c,e) => logger.debug("entering live participantMachine state"), 'flushPrivateDisplayQueue'],
              on: {
                PRIVATE_DISPLAY: {
...
const AnearParticipantMachineFunctions = {
  actions: {
    ...
    queuePrivateDisplayEvents: assign({
      displayQueue: (context, event) => {
        logger.debug("will queue up PRIVATE_DISPLAY event")
    ...
    flushPrivateDisplayQueue: assign({
      displayQueue: (context, event) => {
        if (context.displayQueue.length > 0) {
          logger.debug("send PRIVATE_DISPLAY to machine from displayQueue")
  ...
  guards: {
    ...
    hasMetaTimeout: (context, event) => {
      logger.debug("hasMetaTimeout cond event: ", event)
      logger.debug("hasMetaTimeout cond event data: ", event.data)
      return event.data.timeout.msecs > 0
    }
  },
  delays: {
