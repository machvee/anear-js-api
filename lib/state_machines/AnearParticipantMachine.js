    ...
    active: {
      ...
      states: {
        ...
        live: {
          ...
          // need to transition to a timed (setTimer) wait for ACTION
          // the anearEvent will send the ACTION event when the participant responds
          // waiting For Action state with a delay: timeout.  This can ripple up to the
          // appEventMachine so the developer can deal with how tardy/absent Participants are
        }
      }
    },
    idle: {
...
const AnearParticipantMachineFunctions = {
  actions: {
    ...
    createParticipantTimer: assign(
      {
        participantTimer: context => {
          const sendTimeout = context => context.anearEvent.anearParticipantMachine.send(
            TimeoutEventName,
            {
...
const AnearParticipantMachine = (anearParticipant, { anearEvent, realtimeMessaging }) => {
  ...
  const anearParticipantMachineContext = AnearParticipantMachineContext(anearEvent, anearParticipant, realtimeMessaging)
