          ...
          states: {
            ...
            renderDisplay: {
              ...
              invoke: {
                ...
                onDone: [
                  {
                    cond: 'hasActionTimeout',
                    actions: 'updateActionTimeout',
            ...
            cleanupAndExit: {
              ...
              on: {
                RENDER_DISPLAY: [
                  {
                    cond: 'isExitDisplay',
                    target: '#done'  // Allow exit displays to proceed to final state
                  },
                  {
                    actions: (c, e) => logger.debug('APM ignoring non-exit RENDER_DISPLAY during cleanup')
                  }
                ],
                PRIVATE_DISPLAY: [
                  {
                  ...
                  {
                    actions: (c, e) => logger.debug('APM ignoring non-exit PRIVATE_DISPLAY during cleanup')
                  }
                ],
                PARTICIPANT_EXIT: {
                  actions: (c, e) => logger.debug('APM ignoring redundant PARTICIPANT_EXIT during cleanup')
                },
                ACTION: {
                  actions: (c, e) => logger.debug('APM ignoring ACTION during cleanup')
                }
              },
              invoke: {
              ...
              on: {
                RENDER_DISPLAY: [
                  {
                    cond: 'isExitDisplay',
                    actions: (c, e) => logger.debug('APM allowing exit RENDER_DISPLAY in final state')
                  },
                  {
                    actions: (c, e) => logger.debug('APM ignoring non-exit RENDER_DISPLAY in final state')
                  }
                ],
                PRIVATE_DISPLAY: [
                  {
                    cond: 'isExitDisplay',
                    actions: (c, e) => logger.debug('APM allowing exit PRIVATE_DISPLAY in final state')
                  },
                  {
                    actions: (c, e) => logger.debug('APM ignoring non-exit PRIVATE_DISPLAY in final state')
                  }
                ],
                '*': {
                  actions: (c, e) => logger.debug('APM ignoring event in final state:', e.type)
                }
              },
              type: 'final'
            }
...
const AnearParticipantMachineFunctions = {
  ...
  services: {
    publishPrivateDisplay: async (context, event) => {
      ...
      if (event.exit) displayMessage.exit = true
  ...
  guards: {
    ...
    hasActionTimeout: (_c, event) => event.data.timeout > 0,
    isExitDisplay: (_c, event) => event.data.exit === true
  },
  delays: {
    actionTimeout: context => context.actionTimeoutMsecs,
    reconnectTimeout: _c => (30 * 1000) // they must reconnect in 30 seconds
  }
}
