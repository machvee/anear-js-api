...
const ConnectionMachineFunctions = connectionMachine => ({
  action: {
    ...
    createNewChannel: (context, event) => {
      ...
      assign({ channelMachines: (context, event) => ({
          ...context.channelMachines,
          [event.channelName]: machine
        })
      })
      machine.startService()
    },
    deleteChannel: assign({
      channelMachines: (context, event) => {
        // Create a copy of the channels object without the specified channelName
        const updatedChannels = { ...context.channelMachines }
        delete updatedChannels[event.channelName]
