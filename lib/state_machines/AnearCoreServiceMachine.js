    ...
    loadPugFiles: assign(
      {
        pugTemplates: context => {
          const pugLoader = new PugLoader(DefaultTemplatesRootDir, context.imageAssetsUrl)
    ...
    startNewEventMachine: assign(
      {
        anearEventMachines: (context, event) => {
          ...
          anearEvent.anearEventMachine = service

          service.start()

          return {
            ...context.anearEventMachines,
            [anearEvent.id]: service
          }
        }
      }
    ),
...
const AnearCoreServiceMachine = (appEventMachineFactory, appParticipantMachineFactory = null) => {
  ...
  coreServiceMachineStarted.send(C.UpdateContextEventName, { coreServiceMachine: coreServiceMachineStarted })
