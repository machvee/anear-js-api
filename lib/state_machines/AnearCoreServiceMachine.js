  ...
  actions: {
    ...
    subscribeCreateEventMessages: context => {
      context.newEventCreationChannel.subscribe(
        'CREATE_EVENT',
        message => context.coreServiceMachine.send(message.name, { message })
      )
    },
...
const AnearCoreServiceMachine = appEventMachineFactory => {
  ...
  const coreServiceMachine = createMachine(expandedConfig, AnearCoreServiceMachineFunctions).withContext(anearCoreServiceMachineContext)
