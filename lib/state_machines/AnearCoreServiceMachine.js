...
const AnearCoreServiceMachineContext = (appId, anearApi, AppEventMachineClass) => ({
  ...
  anearEventMachines: {}, // All concurrent events handled by this core service
  createNewEventChannel: null,
  anearApi,
...
const AnearCoreServiceMachineConfig = {
  ...
  states: {
    ...
    initRealtimeMessaging: {
      ...
      always: [
        { target: 'waitRealtimeConnected' }
      ]
    },
    waitRealtimeConnected: {
      on: {
        CONNECTED: {
          actions: ['createCreateEventsChannel', 'subscribeCreateEventMessages'],
          target: 'waitCreateChannelAttached'
        }
      }
    },
    waitCreateChannelAttached: {
      on: {
        ATTACHED: {
...
const AnearCoreServiceMachineFunctions = coreServiceMachine => ({
  ...
  actions: {
    ...
    createCreateEventsChannel: assign({createNewEventChannel: context => {
        const channelName = coreServiceMachine.createNewEventChannelName(context.appId)
        return context.realtimeMessaging.getChannel(channelName, coreServiceMachine, {})
      }
    }),
    subscribeCreateEventMessages: context => {
      return context.createNewEventChannel.subscribe(
        'CREATE_EVENT',
        message => coreServiceMachine.send(message.type, { message })
      )
    },
...
class AnearCoreServiceMachine extends AnearBaseMachine {
  ...
  createNewEventChannelName(appId) {
    return CreateEventChannelNameTemplate(appId)
  }
}
