...
const AnearCoreServiceMachineContext = (appId, anearApi, realtimeMessaging, machineRef, appEventMachineFactory) => ({
  appId,
  machineRef,
  appData: null,
  anearApi,
  ...
  states: {
    ...
    initRealtimeMessaging: {
      ...
      on: {
        ...
        ATTACHED: 'waitAnearEventLifecycleCommand'
      }
    },
...
const AnearCoreServiceMachineFunctions = ({
  ...
  actions: {
    initRealtime: context => context.realtimeMessaging.initRealtime(context.machineRef),
    ...
    createEventsCreationChannel: assign(
      {
        newEventCreationChannel: context => {
          ...
          return context.realtimeMessaging.getChannel(channelName, context.machineRef, {})
        }
      }
    ),
    subscribeCreateEventMessages: context => {
      context.newEventCreationChannel.subscribe(
        'CREATE_EVENT',
        message => context.machineRef.send(message.name, { message })
      )
    },
    startNewEventMachine: assign(
      {
        anearEventMachines: (context, event) => {
          ...
          anearEvent.anearEventMachine = AnearEventMachine(
            anearEvent,
            context.realtimeMessaging,
            context.appEventMachineFactory
          )
...
const AnearCoreServiceMachine = appEventMachineFactory => {
  ...
  const coreServiceMachine = createMachine(expandedConfig, AnearCoreServiceMachineFunctions)

  const contextWithMachineRef = AnearCoreServiceMachineContext(
    appId,
    anearApi,
    realtimeMessaging,
    coreServiceMachine,
  ...
  return serviceMachine.withContext(contextWithMachineRef)
}
