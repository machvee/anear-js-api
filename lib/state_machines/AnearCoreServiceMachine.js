...
const AnearCoreServiceMachineFunctions = ({
  services: {
    ...
    buildAndMinifyCss: context => CssProcessing(Constants.CssDirPath, context.imageAssetsUrl),
    uploadCssForClientAccess: (context, event) => context.anearApi.uploadAppCss(
      context.appId,
      context.css,
      context.cssFileSuffix
    ),
    imagesArchiver: context => new ImagesArchiver(Constants.ImagesDirPath).createTarball(),
