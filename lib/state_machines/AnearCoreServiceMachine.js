...
const CssProcesing = require('../utils/CssProcessing')
const ImagesArchiver = require('./ImagesArchiver')
  ...
  states: {
    ...
    archiveImageAssets: {
      invoke: {
        ...
        onError: '#error' // Handle error state or transition
      }
    },
    uploadImageArchive: {
      invoke: {
    ...
    buildAndMinifyCSS: {
      invoke: {
        ...
        onDone: {
          actions: assign({
            css: (_, event) => event.data.minifiedCss,
...
const AnearCoreServiceMachineFunctions = ({
  services: {
    ...
    buildAndMinifyCSS: context => CssProcessing(context.imageAssetsUrl),
    ...
    updateAppCssUrl: assign(
      {
        ...
        minifiedCss: _ => null // uploaded and not needed locally
      }
    ),
  ...
  guards: {
    anyImageAssetData: (_, event) => event.data === null
  }
})
