  ...
  actions: {
    ...
    processDisconnectEvents: (context, event) => {
      ...
      if (participantMachine) {
        participantMachine.send(eventName);
      }
    ...
    processUpdateEvents: (context, event) => {
      ...
      const eventName = isHost ? 'HOST_UPDATE' : 'PARTICIPANT_UPDATE';
      const participantMachine = context.participantMachines[id];
      const payload = { type: eventName, participant };

      logger.debug(`[AEM] processing ${eventName} for ${id}`);
      if (participantMachine) {
        participantMachine.send(payload);
      }
      context.appEventMachine.send(payload);
    },
