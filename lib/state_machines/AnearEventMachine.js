    ...
    live: {
      ...
      states: {
        ...
        waitAllParticipantsResponse: {
          id: 'waitAllParticipantsResponse',
          always: {
            cond: 'allParticipantsResponded',
            target: 'waitingForActions',
            actions: ['sendActionCompleteToAppM', 'clearParticipantsTimeout']
          },
          after: {
          ...
          on: {
            ACTION: {
              actions: ['processParticipantResponse', 'processParticipantAction'],
              internal: true
            },
            PARTICIPANT_LEAVE: [
              {
                cond: 'isPermanentLeave',
                actions: ['removeLeavingParticipantFromTimeout', 'sendParticipantExitToAppEventMachine', 'sendExitToParticipantMachine']
              },
              {
                actions: ['sendParticipantDisconnectEvents']
              }
        ...
        handleParticipantsTimeout: {
          entry: ['sendTimeoutToNonResponders', 'clearParticipantsTimeout'],
...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    startNewParticipantMachine: assign((context, event) => {
      ...
      // Determine if this participant should be assigned as the host
      const isHostedEvent = context.anearEvent.hosted
      const noHostAssignedYet = !context.hostId
      const thisParticipantIsHost = anearParticipant.isHost()
      const shouldAssignAsHost = isHostedEvent && noHostAssignedYet && thisParticipantIsHost

      // Assign host ID: only the first host participant in a hosted event becomes the host
      // Once a host is assigned, it should never change
      const newHostId = shouldAssignAsHost ? anearParticipant.id : context.hostId
    ...
    sendParticipantEnterToAppEventMachine: (context, event) => {
      const anearParticipant = event.data ? event.data.anearParticipant : event.anearParticipant;
      if (!anearParticipant) {
        logger.error('[AEM] sendParticipantEnterToAppEventMachine called without an anearParticipant in the event');
        return;
      }
      ...
      // If this participant is the host, also send HOST_ENTER event
      if (anearParticipant.id === context.hostId) {
        const hostEvent = {
          participantId: anearParticipant.id,
          participantInfo: participantInfo
        }
        context.appEventMachine.send('HOST_ENTER', hostEvent)
      }
    },
    ...
    sendParticipantExitToAppEventMachine: (context, event) => {
      ...
      // If this participant is the host, also send HOST_EXIT event
      if (event.data.id === context.hostId) {
        context.appEventMachine.send('HOST_EXIT', { participantId: event.data.id })
      }
    },
    ...
    sendParticipantDisconnectEvents: (context, event) => {
      ...
      if (participantMachine) {
        ...
        // If this participant is the host, also send HOST_DISCONNECT event
        if (event.data.id === context.hostId) {
          context.appEventMachine.send('HOST_DISCONNECT', { participantId: event.data.id })
        }
      }
    },
    sendParticipantReconnectEvents: (context, event) => {
      ...
      if (participantInfo) {
        ...
        // If this participant is the host, also send HOST_RECONNECT event
        if (participantId === context.hostId) {
          context.appEventMachine.send('HOST_RECONNECT', { participantId: participantId })
        }
      }
    },
    // HOST Events - Send dedicated HOST events to AppM for cleaner host handling
    sendHostEnterToAppEventMachine: (context, event) => {
      const anearParticipant = event.data ? event.data.anearParticipant : event.anearParticipant;
      if (!anearParticipant) {
        logger.error('[AEM] sendHostEnterToAppEventMachine called without an anearParticipant in the event');
        return;
      }
      const participantInfo = context.participants[anearParticipant.id]

      logger.debug(`[AEM] Host machine ${participantInfo.id} is READY`)
    ...
    sendHostDisconnectToAppEventMachine: (context, event) => {
      const participantId = event.data.id
      if (participantId === context.hostId) {
        logger.debug("[AEM] sending HOST_DISCONNECT to AppM")
        context.appEventMachine.send('HOST_DISCONNECT', { participantId: participantId })
      }
    },
    sendHostReconnectToAppEventMachine: (context, event) => {
      const participantId = event.data.id
      if (participantId === context.hostId) {
        logger.debug("[AEM] sending HOST_RECONNECT to AppM")
        context.appEventMachine.send('HOST_RECONNECT', { participantId: participantId })
      }
    },
    sendHostExitToAppEventMachine: (context, event) => {
      const participantId = event.data.id
      if (participantId === context.hostId) {
        logger.debug("[AEM] sending HOST_EXIT to AppM")
        context.appEventMachine.send('HOST_EXIT', { participantId: participantId })
      }
    },
    ...
    sendActionCompleteToAppM: context => {
      if (context.appEventMachine) {
        logger.debug('[AEM] All participants responded. Sending ACTION_COMPLETE to AppM.')
        context.appEventMachine.send('ACTION_COMPLETE', {
          timedOut: false,
          nonResponderIds: []
        })
      }
    ...
    setupParticipantsTimeout: assign((context, event) => {
      ...
      const allParticipantIds = Object.keys(context.participants)
    ...
    processParticipantResponse: assign((context, event) => {
      ...
      nonResponders.delete(participantId)

      return {
        participantsActionTimeout: {
          ...context.participantsActionTimeout,
          nonResponders
        }
      }
    }),
    ...
    sendTimeoutToNonResponders: (context, _event) => {
      const { nonResponders } = context.participantsActionTimeout
