...
const ActiveEventGlobalEvents = {
  PARTICIPANT_EXIT: {
    // If any participant explicitly exits, notify the APM so it can
    // cleanup and shutdown, and we'll catch it's PARTICIPANT_MACHINE_EXIT
    // Also, notify the AppM so it can determine what effect the exiting participant will
    // have on the event in progress.  e.g. If its the event creator and the event has not
    // started yet, the AppM can just shutdown
    actions: [
  ...
  actions: {
    ...
    startNewParticipantMachine: assign(
      (context, event) => {
        ...
        if (context.participants[participantJSON.data.id]) {
          logger.debug(`participant entry for ${participantJSON.data.id} already exists`)
          return {}
        }
        ...
        const participantEntry = {
          ...
          publicAttrs: anearParticipant.publicAttrs,
          privateChannel: null
        }
        ...
        return {
          participants: {
            ...
            [anearParticipant.id]: participantEntry
          }
        }
      }
    ),
    ...
    sendParticipantExitToParticipantMachine: (context, event) => {
      ...
      const participant = context.participants[event.data.participantId]
      if (participant && participant.machine) {
        logger.debug("sending PARTICIPANT_EXIT to ", participant.machine.id)
        participant.machine.send('PARTICIPANT_EXIT', event.data)
      }
    },
    sendParticipantExitEvents: context => {
      Object.values(context.participants).forEach(p => p.machine.send('PARTICIPANT_EXIT'))
    },
    sendParticipantDisconnectEvents: (context, event) => {
      const participant = context.participants[event.data.id]
      if (participant && participant.machine) {
        ...
        participant.machine.send('PARTICIPANT_DISCONNECT', event.data)
    ...
    sendParticipantReconnectEvents: (context, event) => {
      ...
      if (participant && participant.machine) {
        ...
        // the event
        participant.machine.send('PARTICIPANT_RECONNECT', event.data)
      }
    ...
    updateParticipantPresence: (context, event) => {
      // lookup the participant.machine and update their context
      const participant = context.participants[event.data.id]
      if (participant && participant.machine) {
        ...
        participant.machine.send('PARTICIPANT_UPDATE', event.data)
      }
    ...
    processParticipantAction: (context, event) => {
      ...
      participant.machine.send('ACTION', actionEventPayload)
  ...
  services: {
    renderDisplay: async (context, event) => {
      const processDisplayEvent = (context, displayEvent) => {
        ...
        const extractPublicAttrs = participantsMap =>
          Object.fromEntries(
            Object.entries(participantsMap).map(([id, participant]) => [
              id,
              participant.publicAttrs
            ])
          )
          ...
          const renderedMessage = template(
            {
              ...
              participants: extractPublicAttrs(context.participants),
              ...context.pugHelpers
            }
          )
        ...
        const privateParticipantSend = participant => {
          const participantMachine = participant.machine
          ...
          const privateContext = {
            anearParticipant: participantContext.anearParticipant.publicAttrs,
        ...
        switch (displayType) {
          ...
          case 'participant':
            ...
            Object.values(context.participants).forEach(privateParticipantSend)
