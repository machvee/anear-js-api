...
const ActiveEventStatesConfig = {
  ...
  states: {
    ...
    created: {
      entry: ['enableParticipantPresenceEvents', 'enableSpectatorPresenceEvents'],
      on: {
        ANNOUNCE: {
          actions: (c, e) => logger.debug("Got Announce event...spectators can JOIN"),
          target: 'announce'
        },
        START: {
...
const AnearEventMachineFunctions = ({
  ...
  services: {
    ...
    getAttachedSpectators: async (context, event) => {
      return RealtimeMessaging.getPresenceOnChannel(context.spectatorsDisplayChannel)
    },
