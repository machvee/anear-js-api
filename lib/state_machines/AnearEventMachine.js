...
const ActiveEventStatesConfig = {
  ...
  states: {
    ...
    hist: {
      entry: (c, e) => logger.debug("**** hist state: ", e),
      type: 'history',
      history: 'deep'
    }
  }
...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    queueDisplayEvents: assign({
      displayEvents: (context, event) => [...context.displayEvents, ...event.displayEvents]
    })
  },
  services: {
    renderDisplay: async (context, event) => {
      ...
      const publishPromises = context.displayEvents.map(
