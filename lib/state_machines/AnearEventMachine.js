...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    sendTimeoutToNonResponders: (context, _event) => {
      const { nonResponders } = context.participantsActionTimeout
      logger.info(`[AEM] Participants action timed out. Non-responders: ${[...nonResponders].join(', ')}`)

      nonResponders.forEach(participantId => {
        context.appEventMachine.send('PARTICIPANT_TIMEOUT', { participantId })
      })
    },
