...
const RenderDisplayEvent = {
  invoke: {
    src: 'renderDisplay',
    onDone: {
      actions: (c, e) => logger.debug("RENDER_DISPLAY complete: "),
      target: '.',  // Refers to the current state
      internal: true
    },
    onError: {
      target: 'failure'
  ...
  states: {
    ...
    created: {
      ...
      on: {
        ...
        RENDER_DISPLAY: RenderDisplayEvent
      }
    },
...
const AnearEventMachineFunctions = ({
  ...
  services: {
    renderDisplay: async (context, event) => {
      const { displayType, viewPath, timeout, executionContext } = event

      logger.debug("displayType: ", displayType, "viewPath: ", viewPath)
      
      const renderedDisplayContent = (template, context, timeout) => {
        const renderedMessage = template(context)
      ...
      const privateParticipantSend = async machine => {
        const participantState = machine.state

        if (!machine.state.matches(C.ActiveStateName)) {
          return Promise.resolve()
        }
        const privateContext = {
          anearParticipant: machine.context.anearParticipant,
          participantContext: participantState.context,
          ...executionContext
        }
        const displayMessage = renderedDisplayContent(template, privateContext, timeout)

        logger.debug("rendered Message: ", displayMessage)

        return context.participantsDisplayChannel.publish(C.ParticipantDisplayEventName, displayMessage)
      }
      ...
      switch (displayType) {
        case 'participants':
          ...
          return context.participantsDisplayChannel.publish(C.ParticipantsDisplayEventName, displayContent)
          break
        case 'spectators':
          displayMessage = renderedDisplayContent(template, executionContext)
          return context.spectatorsDisplayChannel.publish(C.SpectatorsDisplayEventName, displayMessage)
          break
        case 'participant':
          // For private displays, iterate over active active participant machines.
          const promises = Object.values(context.anearParticipantMachines).map(privateParticipantSend)
