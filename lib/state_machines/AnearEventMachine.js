...
const GlobalEventConfig = { // STILL NEED THIS?   We are handling events locally for better transition context
  // target '.' or deep history transitions have NOT been working too well
  // these are toast
  PARTICIPANT_TIMEOUT: {
    //
    ...
    announce: {
      ...
      on: {
        ...
        PARTICIPANT_EXIT: {
          ...
          actions: ['sendParticipantExitToAppEventMachine', 'removeParticipant'],
          target: '#activeEvent.canceled'
        }
      },
      states: {
...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    processParticipantAction: (context, event) => {
      ...
      anearParticipantMachine.send('ACTION', actionEventPayload)
    ...
    renderDisplay: async (context, event) => {
      const processDisplayEvent = (context, displayEvent) => {
        ...
        const privateParticipantSend = anearParticipantMachine => {
          ...
          anearParticipantMachine.send('PRIVATE_DISPLAY', displayEventPayload)
        }
