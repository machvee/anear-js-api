...
const ActiveEventGlobalEvents = {
  PARTICIPANT_EXIT: {
    ...
    // cleanup and shutdown, and we'll catch it's done.invoke.
  ...
  // This wildcard will catch done.invoke.anearParticipantMachine_<id>
  'done.invoke.anearParticipantMachine_*': {
    actions: 'cleanupExitingParticipant'
  },
  'done.invoke.appEventMachine': {
    actions: '#cleanupAndExit'
  }
}
  ...
  states: {
    ...
    announce: {
      ...
      states: {
        ...
        newParticipant: {
          // a PARTICIPANT_ENTER recieved from a new user JOIN click. Spawn an AnearParticipantMachine instance.
  ...
  actions: {
    ...
    createAppEventMachine: assign(
      {
        appEventMachine: context => {
          ...
          service.subscribe(MetaProcessing(context.anearEvent))
          const actor = spawn(service, "appEventMachine")
    ...
    startNewParticipantMachine: assign(
      (context, event) => {
        ...
        const actor = spawn(service, `anearParticipantMachine_${anearParticipant.id}`)

        return {
          anearParticipantMachines: {
            ...context.anearParticipantMachines,
            [anearParticipant.id]: actor.start()
          },
          participants: {
    ...
    sendParticipantExitToAppEventMachine: (context, event) => {
      context.appEventMachine.send('PARTICIPANT_EXIT', event.data)
    },
    sendParticipantExitToParticipantMachine: (context, event) => {
      // coming from an ACTION message, event.data.participantId
      const participantMachine = context.anearParticipantMachines[event.data.participantId]
    ...
    sendParticipantExitEvents: context => {
      Object.values(context.anearParticipantMachines).forEach(m => m.send('PARTICIPANT_EXIT'))
    },
    sendParticipantDisconnectEvents: (context, event) => {
      const participantMachine = context.anearParticipantMachines[event.data.id]
      if (participantMachine) {
    ...
    processParticipantAction: (context, event) => {
      ...
      const participantMachine = context.anearParticipantMachines[participantId]
    ...
    cleanupExitingParticipant: assign((context, event) => {
      // event.type === "done.invoke.anearParticipantMachine_<participantId>"
      const [, participantId] = event.type.match(/^done\.invoke\.anearParticipantMachine_(.+)$/)

      if (context.participants[participantId]) {
        const {
        ...
        return {
          anearParticipantMachines: remainingMachines,
