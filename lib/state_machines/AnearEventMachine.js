...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    startNewParticipantMachine: assign((context, event) => {
      ...
      // If this is a hosted event and we haven't identified the host yet,
      // this first participant is the host.
      const newHostId = (context.anearEvent.hosted && !context.hostId) ?
        anearParticipant.id :
        context.hostId
    ...
    sendTimeoutToNonResponders: (context, _event) => {
      ...
      if (nonResponderIds.length > 0) {
        context.appEventMachine.send('ACTION_TIMEOUT', { participantIds: nonResponderIds })
      }
    },
