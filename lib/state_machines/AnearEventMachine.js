...
const ActiveEventGlobalEvents = {
  ...
  RENDER_DISPLAY: {
    actions: assign({
      lastDisplayEvents: (context, event) => event.displayEvents,
    }),
  },
  APP_FINAL: {
    // AppM has reached a final state, initiate orderly shutdown
    // if it already closed, do nothing
...
const AnearEventMachineFunctions = ({
  ...
  services: {
    ...
    renderFinalDisplay: (context, event) => {
      if (!context.lastDisplayEvents) {
        logger.debug("no lastDisplayEvents to render. skipping.")
        return Promise.resolve()
      }
      const displayEventProcessor = new DisplayEventProcessor(context)
      return displayEventProcessor.processAndPublish(context.lastDisplayEvents)
    },
