...
const GlobalEventConfig = {
  ...
  'done.invoke.anearParticipantMachine_*': {
    actions: 'cleanupParticipantMachine'
  }
}
  ...
  states: {
    ...
    eventCreated: {
      ...
      on: {
        ...
        PARTICIPANT_EXIT: {
          // The event Creator/host elected to permanently leave event from their device
          actions: ['sendParticipantExitToAppEventMachine'],
          target: '#activeEvent.canceled'
        }
      },
      states: {
        waitingAnnounce: {
    ...
    announce: {
      ...
      on: {
        ...
        PARTICIPANT_LEAVE: {
          ...
          actions: [
            (_c, e) => logger.debug("PARTICIPANT_LEAVE presence event received: ", e.data),
            'sendParticipantDisconnectEvents'
          ],
        ...
        PARTICIPANT_EXIT: {
          // A Participant elected to permanently leave event from their device
          actions: ['sendParticipantExitToAppEventMachine'],
          target: '#activeEvent.canceled'
        }
      },
...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    cleanupParticipantMachine: assign((context, event) => {
      ...
      // Omit the participant machine
      const {
        [participantId]: removedMachine,
        ...remainingMachines
      } = context.anearParticipantMachines

      // Omit the participant publicAttrs
      const {
