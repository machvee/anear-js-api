  ...
  states: {
    ...
    created: {
      entry: [
        ...
        (c, e) => logger.debug("created state entered")
      ],
      invoke: {
        src: 'getAndNotifyAttachedCreatorOrHost',
        onDone: {
          target: 'created',
          internal: true
        },
...
const AnearEventMachineFunctions = ({
  ...
  services: {
    ...
    publishSpectatorsDisplay: (context, event) => RealtimeMessaging.publish(
      context.spectatorsDisplayChannel,
      C.SpectatorsDisplayEventName,
      event.content
    ),
    getAndNotifyAttachedSpectators: (context, event) => {
      return RealtimeMessaging.getAndNotifyPresenceOnChannel(
        context.spectatorsDisplayChannel,
        context.anearEvent.anearEventMachine,
        SpectatorPresencePrefix
      )
    },
    getAndNotifyAttachedCreatorOrHost: (context, event) => {
      return RealtimeMessaging.getAndNotifyPresenceOnChannel(
        context.actionsChannel,
        context.anearEvent.anearEventMachine,
        C.ParticipantPresencePrefix
    ...
    attachToParticipantsDisplayChannel: async context => {
      return RealtimeMessaging.attachTo(context.participantsDisplayChannel)
    },
    attachToSpectatorsChannel: async context => {
      return RealtimeMessaging.attachTo(context.spectatorsDisplayChannel)
    },
    attachToActionsChannel: async context => {
      return RealtimeMessaging.attachTo(context.actionsChannel)
    },
    attachToEventChannel: async context => {
      return RealtimeMessaging.attachTo(context.eventChannel)
    },
