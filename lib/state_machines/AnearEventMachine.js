...
const GlobalEventConfig = {
  ...
  RENDER_DISPLAY: {
    ...
    actions: ['queueRenderDisplayEvent'],
    target: '.',
    internal: true
  },
...
const AnearEventMachineFunctions = ({
  actions: {
    ...
    queueRenderDisplayEvent: assign({
      displayQueue: (context, event) => [...context.displayQueue, event]
    }),
    ...
    flushDisplayQueue: assign({
      displayQueue: (context, event) => {
        if (context.displayQueue.length > 0) {
          logger.debug("Flushing display queue with ", context.displayQueue.length, " event(s)")
          context.displayQueue.forEach(
            queuedEvent => {
              context.anearEvent.machineRef.send(C.RenderDisplayEventName, queuedEvent)
            }
          )
        }
        return []
